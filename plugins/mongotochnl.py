...
async def send_all_files(client: Client, message: Message):
    ...
    if total_files == 0:
        return await message.reply_text("⚠️ No files found in the database.")
    ...
    async for file_doc in cursor:
        ...
        try:
            # ✅ Decode file_id to get permanent file_ref
            unpacked = FileId.decode(file_id)

            await client.send_cached_media(
                chat_id=BIN_CHANNEL,
                file_id=file_id,
                file_ref=unpacked.file_ref,  # permanent ref
